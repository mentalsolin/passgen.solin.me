...
function calculateEntropy(str) { // Функция подсчета энтропии пароля по формуле Шеннона
    ...
    entropyScoreBox.innerText = `Entropy: ${entropy.toFixed(2)} bit` // Выводит текущее значение энтропии в DOM-дереве
    ...
    if (entropy < 1) {
        entropyQuality.innerText = `Password Quality: Bad`
    } if (entropy >= 1) {
        entropyQuality.innerText = `Password Quality: Poor`
        entropyBar.style.backgroundColor = `var(--quality-poor)`;
    } if (entropy >= 2.7) {
        entropyQuality.innerText = `Password Quality: Weak`
        entropyBar.style.backgroundColor = `var(--quality-weak)`;
    } if (entropy >= 3.5) {
        entropyQuality.innerText = `Password Quality: Good`
        entropyBar.style.backgroundColor = `var(--quality-good)`;
    } if (entropy >= 3.7) {
        entropyQuality.innerText = `Password Quality: Excellent`
